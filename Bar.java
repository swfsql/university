...
public class Bar {
	...
	void collision(Stage stage) {
		...
		boolean red_collision = stage.test_collision(x, y); // tests the middle point
		...
		for (int i = 0; i < collision_points && !new_collision ; i++) {
			...
			red_collision = stage.test_collision(x + (int)(len2 * (i + 1) * Math.cos(rad)), y + (int)(len2 * (i + 1) * Math.sin(rad)),0xFFFF0000);
		}
